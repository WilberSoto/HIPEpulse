...
export default async function ProfilePage({ params }: { params: Promise<{ username: string }> }) {
  ...
  if (user && !isOwner) {
    const { data } = await supabase.from('follows').select('follower_id')
      .match({ follower_id: user.id, following_id: finalProfile.id }).single()
    isFollowing = !!data
  }

  // Posts
  const { data: rawPosts } = await supabase.from('posts_with_meta').select('*')
    .eq('author_id', finalProfile.id).order('created_at', { ascending: false })
  ...
  const postTags = Array.from(new Set(posts.flatMap(p => p.tags ?? []))).sort()
  const eventTags = Array.from(new Set((events ?? []).flatMap((e: any) => e.tags ?? []))).sort() as string[]  
  ...
  return (
    <div style={{ minHeight: '100vh', background: C.bg, fontFamily: font.sans }}>
      ...
      <main style={{ maxWidth: 720, margin: '0 auto', padding: '48px 24px 64px' }}>
        ...
        <div style={{ ...glass, padding: '40px 32px 32px', marginBottom: 32, textAlign: 'center' }}>
          <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 16 }}>
            {finalProfile.avatar_url
              ...
              : <div style={{ width: 88, height: 88, borderRadius: '50%', background: 'rgba(255,255,255,0.1)', border: '2px solid rgba(255,255,255,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 32, color: C.accent }}>
                  {(finalProfile.full_name ?? finalProfile.username ?? '?')[0].toUpperCase()}
                </div>
            }
          </div>

          {finalProfile.full_name && (
            <h1 style={{ fontFamily: font.serif, fontSize: 28, fontWeight: 600, color: C.textPrimary, marginBottom: 4 }}>{finalProfile.full_name}</h1>
          )}
          ...
          {finalProfile.bio && (
            <p style={{ fontSize: 14, color: C.textSecondary, fontWeight: 300, lineHeight: 1.6, maxWidth: 480, margin: '0 auto 18px' }}>{finalProfile.bio}</p>
          )}
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: 28, marginBottom: 20, fontSize: 13 }}>
            ...
            <span><strong style={{ color: C.textPrimary }}>{(events ?? []).length}</strong> <span style={{ color: C.textMuted }}>events</span></span>
        ...
        {/* Posts + Events with separate filters */}
        <ProfileContent
          posts={posts}
          events={events ?? []}
          postTags={postTags}
